...
public class CommonDao<T extends Object> {
    ...
    public CommonDao(Class<T> clz, DbHelper dbHelper) {
        this.dbHelper = dbHelper;
        try {
            commonDao = dbHelper.getDao(clz);
    ...
    public boolean save(List<T> models) {
        try {
            return commonDao.create(models) == 1;
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return false;
...
/*    public void saveOrUpdate(List<T> models) {
        for (int i = 0; i < models.size(); i++) {
            try {
                commonDao.createOrUpdate(models.get(i));
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
    ...
    public boolean delete(List<T> models) {
        try {
            return commonDao.delete(models) == 1;
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return false;
